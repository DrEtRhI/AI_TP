...
function dessinerChFleur() {
    ...
    ctx.clearRect(0, 0, canvas.width, canvas.height); // ajouté par nous pour nettoyer la zone de dessin afin de redessiner
    var rayon = canvas.width / (2 * nombrefleur);
    cx = rayon;
    cy = rayon;

    for (var i = 0; i < nombrefleur; i++) { //bien initialiser les variables i et j sinon il utilise celles déjà établies, nottament utilisation du i déjà initialisé dans dessiner fleur
